...
def conv2d(input_,output_dim, k_h=3, k_w=3, d_h=2, d_w=2, stddev=0.02, stride=1,name="conv2d"):
    with tf.variable_scope(name):
        w = tf.get_variable('w', [k_h, k_w, input_.get_shape()[-1], output_dim],
                            initializer=tf.truncated_normal_initializer(stddev=stddev))
        conv = tf.nn.conv2d(input_, w, strides=[stride, d_h, d_w,stride], padding='SAME')
...
def deconv2d(input_, output_shape, k_h=3, k_w=3, d_h=2, d_w=2, stddev=0.02, stride=1,name="deconv2d", with_w=False):
    with tf.variable_scope(name):
        ...
        deconv = tf.nn.conv2d_transpose(input_, w, output_shape=output_shape, strides=[stride, d_h, d_w, stride])
